...
const colorMap = {
  cyan:   { bg: 'bg-accent-cyan/10',   border: 'border-accent-cyan/20',   icon: 'text-accent-cyan',   glow: '34,211,238' },
...
export function StatCard({ title, value, change, changeLabel, icon: Icon, color = 'cyan', suffix }: StatCardProps) {
  ...
  return (
    <div
      ...
      style={{ boxShadow: `0 0 30px rgba(${colors.glow},0.1)` }}
    >
      ...
      <div
        ...
        style={{ background: `linear-gradient(90deg, transparent, rgba(${colors.glow},0.4), transparent)` }}
      />
    </div>
  )
}
