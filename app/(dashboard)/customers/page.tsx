...
const EMPTY_FORM: Omit<Customer, 'id' | 'createdAt'> = {
  ...
  status: 'lead', serviceType: '',
  notes: '', taxCode: '', insUsername: '', insIdNumber: '', insPassword: '',
}
...
export default function CustomersPage() {
  ...
  const [showInsPassword, setShowInsPassword] = useState(false)
  ...
  const openAdd = () => { setEditingCustomer(null); setForm(EMPTY_FORM); setShowInsPassword(false); setShowForm(true) }
  const openEdit = (c: Customer) => {
    setEditingCustomer(c)
    setForm({ name: c.name, email: c.email, phone: c.phone, company: c.company, status: c.status, serviceType: c.serviceType || '', notes: c.notes, taxCode: c.taxCode || '', insUsername: c.insUsername || '', insIdNumber: c.insIdNumber || '', insPassword: c.insPassword || '' })
    setShowInsPassword(false)
    setShowForm(true)
  }
  ...
  return (
    <div className="min-h-screen">
      <Header title="לקוחות" subtitle="ניהול לקוחות ופרטי גישה" />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={closeForm}>
          <div className="bg-bg-card border border-border-muted rounded-2xl p-6 max-w-lg w-full space-y-5 animate-fade-in max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-text-muted mb-1.5">חברה / עסק</label>
                  <input value={form.company} onChange={e => setForm(f => ({ ...f, company: e.target.value }))} placeholder="שם העסק" className={fieldClass} />
                </div>
                <div>
                  <label className="block text-xs text-text-muted mb-1.5">סטטוס</label>
              ...
              <div>
                <label className="block text-xs text-text-muted mb-1.5">סוג שירות מבוקש</label>
                <select value={form.serviceType} onChange={e => setForm(f => ({ ...f, serviceType: e.target.value }))} className={fieldClass}>
              ...
              <div>
                ...
                <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="הערות על הלקוח..." rows={2} className={`${fieldClass} resize-none`} />
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={form.taxCode || ''} onChange={e => setForm(f => ({ ...f, taxCode: e.target.value }))}
                  placeholder="קוד קבוע..." dir="ltr" className={fieldClass} />
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={form.insUsername || ''} onChange={e => setForm(f => ({ ...f, insUsername: e.target.value }))}
                    placeholder="שם משתמש" dir="ltr" className={fieldClass} />
                </div>
                <div>
                  <label className="block text-xs text-text-muted mb-1.5">מספר ת.ז</label>
                  <input value={form.insIdNumber || ''} onChange={e => setForm(f => ({ ...f, insIdNumber: e.target.value }))}
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showInsPassword ? 'text' : 'password'}
                    value={form.insPassword || ''} onChange={e => setForm(f => ({ ...f, insPassword: e.target.value }))}
                    placeholder="••••••••" dir="ltr" className={`${fieldClass} pl-10`} />
                  <button type="button" onClick={() => setShowInsPassword(!showInsPassword)}
                    className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-secondary transition-colors">
                    {showInsPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
              </div>
            </div>
